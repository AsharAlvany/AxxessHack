  ...
  if (incremental) {
    ...
    for (var patch of incremental) {
      ...
      if (Array.isArray(patch.items)) {
        ...
        for (var _i = 0, _l = patch.items.length; _i < _l; _i++) part[startIndex + _i] = patch.items[_i];
...
async function* fetchOperation(operation, url, fetchOptions) {
  ...
  try {
    ...
    yield makeErrorResult(operation, (response.status < 200 || response.status >= 300) && response.statusText ? new Error(response.statusText) : error, response);
  } finally {
